...
export(PackedScene) var terrain_path
...
func _ready():
	...
	if not "--server" in OS.get_cmdline_args() or Engine.editor_hint:
		if terrain_path:
			var terrain = terrain_path.instance()
			add_child(terrain)
	else:
		
		# TODO : create only collision shape
...
#
# TODO : need to move to specific node
#
...
func set_collision_enabled(enabled: bool):
	print("set_collision_enabled ", enabled)
