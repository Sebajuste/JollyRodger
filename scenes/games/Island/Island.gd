...
#const Palm = preload("res://scenes/objects/Palm/Palm.tscn")
...
export(float) var sand_level = 15
export(float) var terrain_size = 513

#export(int) var max_palms = 500
#export(float) var min_palms_gap = 20
#export(float, 0, 1) var palms_perlin_threshold = 0.6
export(float) var palms_height_threshold = 40
...
func _ready():
	...
	for z in heightMap.get_height():
		for x in heightMap.get_width():
			...
			# Sand depending of the slope and a sand level
			var sand_amount = clamp(1 - slope + sand_level - h, 0.0, 1.0)
			# Rock on the slopes
			var rock_amount = clamp(1.0 - slope, 0.0, 1.0)
			
			# Add details where there is no rock and not too much sand
			var grass_amount = clamp(1 - rock_amount * grass_prct_rock - sand_amount * grass_prct_sand, 0, 1)
			var flower_amount = clamp(1 - rock_amount - sand_amount, 0, 1)
			
			var palm_amount = clamp(h - palms_height_threshold, 0, 1)
			...
			palmMap.set_pixel(x, z, Color(0,0,0,0))
...
func generate_height(x : float, z : float, center : Vector2) -> float :
	var h = (noise.get_noise_2d(x, z) + 1) / 2
