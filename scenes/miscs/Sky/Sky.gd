...
export (GradientTexture) var fog_gradient_texture
...
func set_time():
	...
	if fog_gradient_texture:
		var fog_color : Color = fog_gradient_texture.gradient.interpolate(time_of_day)
		env.set_fog_color(fog_color)
